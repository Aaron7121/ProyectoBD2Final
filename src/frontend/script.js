...
async function search(query, filter) {
    ...
    try {
        showLoadingIndicator('Buscando...');
        showSection('search-results'); // Mostrar la sección de resultados inmediatamente

        // Si estamos buscando artistas, verificar primero
        if (filter === 'artists') {
            const checkResponse = await fetch(`/api/artists/check?name=${encodeURIComponent(query)}`);
            const checkResult = await checkResponse.json();

            if (!checkResult.exists) {
                hideLoadingIndicator();
                displayNoArtistFound(query);
                return;
            }
        }
        ...
        // Siempre mostrar resultados, incluso si están vacíos
        displaySearchResults(results, filter);

    } catch (error) {
        hideLoadingIndicator();
        console.error('Error en búsqueda:', error);
        // Mostrar error como resultado de búsqueda en lugar de alert
        displaySearchError(error.message);
    }
}

// Añadir esta nueva función para mostrar errores
function displaySearchError(errorMessage) {
    const container = document.getElementById('search-results-container');
    container.innerHTML = `
        <div class="search-error">
            <p>Error en la búsqueda: ${errorMessage}</p>
...
function displaySearchResults(results, filter) {
    ...
    // Manejar el caso de artista no encontrado
    if (results.length === 0 && filter === 'artists') {
        displayNoArtistFound(document.getElementById('search-input').value);
        return;
    }

    // Manejar resultados vacíos para otros tipos de búsqueda
...
async function handleAddNewArtist(artistName) {
    if (!artistName.trim()) {
        alert('Por favor ingrese un nombre de artista válido');
        return;
    }

    try {
        showLoadingIndicator('Verificando artista...');

        // Verificar existencia
        const checkResponse = await fetch(`/api/artists/check?name=${encodeURIComponent(artistName)}`);
        const checkResult = await checkResponse.json();

        if (checkResult.exists) {
            hideLoadingIndicator();
            showNotification('El artista ya existe en la base de datos', 'info');
            return;
        }

        showLoadingIndicator('Añadiendo artista...');
        const addResponse = await fetch('/api/artists/add', {
            method: 'POST',
        ...
        if (addResponse.ok) {
            showNotification(`¡Artista ${artistName} añadido exitosamente!`);
            showNotification('Las letras se cargarán en segundo plano', 'info');
            
            // Mostrar los resultados inmediatamente
            if (addResult.artist) {
                displaySearchResults([addResult.artist], 'artists');
                showSection('search-results');
            }
        } else {
            throw new Error(addResult.message || 'Error al añadir el artista');
        }
    } catch (error) {
        hideLoadingIndicator();
...
// Modificar la función showNotification para soportar diferentes tipos
...
function displayNoArtistFound(artistName) {
    const container = document.getElementById('search-results-container');
    container.innerHTML = `
        <div class="no-results">
            <p>No se encontró el artista "${artistName}"</p>
            <button onclick="handleAddNewArtist('${artistName}')">
                Añadir artista "${artistName}"
            </button>
        </div>
    `;
}
